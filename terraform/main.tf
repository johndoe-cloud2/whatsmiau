...
resource "aws_db_instance" "main" {
  ...
  # Asegurar que se puede destruir sin problemas
  lifecycle {
...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode([{
    ...
    healthCheck = {
      command     = ["CMD-SHELL", "wget --no-verbose --tries=1 --spider http://localhost:8081/health 2>/dev/null || exit 1"]
      interval    = 30
      timeout     = 5
      retries     = 3
      startPeriod = 60
...
resource "aws_ecs_service" "main" {
  ...
  # Permitir destrucción sin problemas
  lifecycle {
    create_before_destroy = false
