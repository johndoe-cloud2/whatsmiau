...
ECR_REGISTRY="${ECR_REGISTRY:?Set ECR_REGISTRY or AWS_ACCOUNT_ID in aws-config.env}"
ECR_REPO_BACKEND="${ECR_REPO_BACKEND:-whatsmiau}"
ECR_REPO_ROUTER="${ECR_REPO_ROUTER:-whatsmiau-router}"
API_SECRET="${API_SECRET:?Set API_SECRET in aws-config.env}"
WEBHOOK_URL="${WEBHOOK_URL:?Set WEBHOOK_URL in aws-config.env}"
